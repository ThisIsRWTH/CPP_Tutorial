...
void Datum::eingabe(std::istream& in)
{
	...
	while (true) // Endlosschleife
	{
		...
		try
		{
			...
			// Überprüfen des richtigen Jahrhunderts
			if (jahr > 30)
			{
				jahr += 1900; // Datum liegt im letzten Jahrhundert
			}
			else
			{
				jahr += 2000; // Datum liegt in diesem Jahrhundert
			}
...
Datum Datum::operator+(const int anzahl_Tage) const
{
	...
	while (tagNeu > tageMonat[monatNeu-1])
	{
		...
		// Monat darf nicht groesser als 12 werden
		if (monatNeu > 12)
		{
			monatNeu -= 12;
			jahrNeu++;
